...
import tokenizer
from utils_e.utils_file import load_glove, write_file
...
class VTTExample(object):
    ...
    def __init__(self, data_type, model_params, dicts=None, is_binary=True):
        ...
        self.token2index = dict([(token,idx) for idx, token in enumerate(list(dicts['token'] + dicts['glove']))])
        self.char2index = dict([(char,idx) for idx, char in enumerate(list(dicts['char']))])

        self.emb_mat_token, self.emb_mat_glove = None, None

        # Number of output classes
        self.max_ans_size = 2
        self.max_token_size = self.model_params.max_token_size
    ...
    def prepare_input(self, data_list):
        ...
        self.max_sentence_size = self.model_params.max_sentence_size
        self.max_token_size = self.model_params.max_token_size
        
        sta = time.time()
        self.digitized_data_list = self.digitize_dataset(data_list, [self.token2index, self.char2index])
        end = time.time()
        #print("digitize : %f (s)" % (end - sta))
    ...
    def index2vector(self):
        ...
        tf.compat.v1.logging.info('mat_token_len : %d , mat_glove_len : %d' %(len(mat_token), len(mat_glove)))
