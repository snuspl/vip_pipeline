...
class FriendsBertDataset(Dataset):
    def __init__(self, samples, tokenizer,transform=None):
        self.samples = samples#pd.read_csv(csv_file)
    ...
    def __getitem__(self, idx):
        ...
        sample = {'input1':(input1,input1_segment), 'input2':(input2,input2_segment)}
...
class FriendsBertDataLoader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, shuffle=True, validation_split=0.0, num_workers=1, tokenizer = None):
        """
        trsfm = transforms.Compose([
            transforms.ToTensor()
        ])
