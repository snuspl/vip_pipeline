...
def generate_samples(dataset, scene, question):
    cnt = 0
    sample_list = []

    for x in dataset:
        if x['name'].split('_')[0] == scene:
            row_element = {"index":cnt, "shot_id":x['name'], "question":question, "subtitles":x['subtitle'], "shot_descs":x['shot_desc'], "label":"1"}
            sample_list.append(row_element)
            cnt += 1
